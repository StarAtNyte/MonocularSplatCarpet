...
export default class MyRoomHelper {
  ...
  calculateCarpetPosition(analysisData) {
    ...
    try {
      ...
      // Fallback if raycasting fails - try to use center of view as fallback
      ...
      console.error('Error in calculateCarpetPosition:', error);
      return [0, 0, 0];
    }
  }
  ...
  _estimateFloorBounds() {
    ...
    try {
      ...
      // Add small padding to bounds (3% inset from edges to prevent edge clipping)
      const paddingX = (maxX - minX) * 0.03;
      const paddingZ = (maxZ - minZ) * 0.03;
  ...
  _estimateCarpetDimensions() {
    ...
    try {
      ...
      console.error('Error in _estimateCarpetDimensions:', error);
      return null;
    }
  }
  _isRoomLandscape() {
    // Determine if the room is landscape based on floor bounds or image dimensions
    const floorBounds = this._getFloorBoundsCached();

    if (floorBounds) {
      // Use floor bounds to determine orientation
    ...
    if (this.origWid && this.origHgt) {
      return this.origWid > this.origHgt;
    }
  ...
  _isCarpetLandscape() {
    // Determine if the carpet is landscape based on its physical dimensions
    if (!this.designDetails) {
      return true;
    }
    ...
    try {
      ...
      console.error('Error in _isCarpetLandscape:', error);
      return true;
    }
  }
  ...
  calculateCarpetRotation(analysisData) {
    ...
    try {
      ...
      let horAngle = parseFloat(orientation.hor);

      // Snap horizontal angle to 0° if very small (likely measurement noise or error)
      // 5-10° tolerance handles room analysis errors for horizontal rooms
      const horAngleSnapTolerance = 10; // Increased to handle analysis errors
      if (Math.abs(horAngle) < horAngleSnapTolerance) {
        horAngle = 0;
      }
      // Also snap if very close to 90° (likely meant to be perpendicular)
      if (Math.abs(horAngle - 90) < horAngleSnapTolerance) {
        horAngle = 90;
      }
      if (Math.abs(horAngle + 90) < horAngleSnapTolerance) {
        horAngle = -90;
      ...
      // Check if carpet orientation matches room orientation
      const roomIsLandscape = this._isRoomLandscape();
      const carpetIsLandscape = this._isCarpetLandscape();

      // If orientations don't match, rotate carpet by 90 degrees
      if (roomIsLandscape !== carpetIsLandscape) {
        rotZ += 90;
      }
  ...
  setinitialOrientation(res) {
    ...
    // If carpet is already loaded, update its rotation with the new room analysis data
    if (this.carpetMesh && this.fbxLoaded) {
      const newRotation = this.calculateCarpetRotation(res);
      this.carpetMesh.rotation.fromArray(convertArrIntoRad(newRotation));
      this.initCarpetRot = newRotation;

      // Also update position if needed
      const newPosition = this.calculateCarpetPosition(res);
      this.carpetMesh.position.fromArray(newPosition);
      this.initCarpetPos = newPosition;
    ...
    // Apply same snapping tolerance as carpet rotation to prevent small angle errors
    let horAngleForCamera = parseFloat(res.orientation.hor);
    const horAngleSnapTolerance = 10;
    if (Math.abs(horAngleForCamera) < horAngleSnapTolerance) {
      horAngleForCamera = 0;
    } else if (Math.abs(horAngleForCamera - 90) < horAngleSnapTolerance) {
      horAngleForCamera = 90;
    } else if (Math.abs(horAngleForCamera + 90) < horAngleSnapTolerance) {
      horAngleForCamera = -90;
    }

    const hor_rot = ((90 - horAngleForCamera) * Math.PI) / 180;
